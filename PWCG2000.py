...
while True:
   ...
   /_/     |__/|__/\____/\____//____/\____/\____/\____/ !
        ...
        MAKE A CHOICE:
        -(C)heck existing password against HIBP database
        -(G)enerate a new password
        -(L)anguage selection (yet to be implemented)
        -E(x)it""")                                                                                                     #Note that language selection will be in a later feature branch.
    ...
    if userchoice == "C" or userchoice == "c":
        ...
        if count > 0:
            print(f"Password was found in {count} leaks!")
        else:
            print("Password was not found to be in any known leaks.")

        input("Press RETURN to continue.")                                                                              #Takes the user back to the menu.
